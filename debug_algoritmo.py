"""
Test para depurar el algoritmo MotoIdealRecommender.
"""
import pandas as pd
import sys
import os
...
try:
    ...
    for user_id in usuarios['user_id']:
        try:
            ...
            print(f"  - {user_id}: {cf_recs if cf_recs else 'Sin recomendaciones'}")
        except Exception as e:
            print(f"  - Error con {user_id}: {str(e)}")
            traceback.print_exc()
    ...
    for user_id in usuarios['user_id']:
        try:
            ...
            print(f"  - {user_id}: {cb_recs if cb_recs else 'Sin recomendaciones'}")
        except Exception as e:
            print(f"  - Error con {user_id}: {str(e)}")
            traceback.print_exc()
    ...
    for user_id in usuarios['user_id']:
        try:
            ...
            print(f"  - {user_id}: {hybrid_recs if hybrid_recs else 'Sin recomendaciones'}")
        except Exception as e:
            print(f"  - Error con {user_id}: {str(e)}")
            traceback.print_exc()
    ...
    for user_id in usuarios['user_id']:
        try:
            ...
            for moto_id, score, reasons in moto_ideal_recs:
                ...
                print(f"  - {moto_info['modelo']} ({moto_info['marca']})")
