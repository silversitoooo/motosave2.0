def get_label_propagation_recommendations(user_id, db_session, top_n=5):
    ...
    """
    try:
        # 1. PASO ORIGINAL: Obtener amigos del usuario
        ...
        # 2. PASO ORIGINAL: Obtener interacciones del usuario y sus amigos
        ...
        # 3. PASO ORIGINAL: Generar recomendaciones con label propagation
        ...
        if len(recommendations) < top_n:
            ...
            additional_motos_result = db_session.run("""
                ...
                ORDER BY m.created_at DESC, m.popularity DESC
                LIMIT $limit
        ...
        logger = get_logger()
        logger.error(f"Error al generar recomendaciones con label propagation para {user_id}: {str(e)}")
        return []