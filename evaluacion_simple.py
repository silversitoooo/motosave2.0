...
print("=== EVALUACIÓN DE PRECISIÓN DEL ALGORITMO MOTOIDEALRECOMMENDER ===\n")
...
for user_id, expected_motos in expectativas.items():
    ...
    print(f"- Experiencia: {user_profile['experiencia']}")
    print(f"- Uso previsto: {user_profile['uso_previsto']}")
    print(f"- Presupuesto: {user_profile['presupuesto']}€\n")
    ...
    print(f"Motos esperadas: {', '.join(expected_motos)}")
    print(f"Motos recomendadas: {', '.join(recomendadas)}")
    print(f"Aciertos: {len(aciertos)}/{top_n} (Precisión: {precision*100:.1f}%)\n")
    
    # Detalles de las recomendaciones
    print("Detalle de recomendaciones:")
    for moto_id, score, reasons in recomendaciones:
        ...
        if moto_id in expected_motos:
            print(f"  ✓ Recomendación correcta\n")
        else:
            print(f"  ✗ No coincide con lo esperado\n")
...
# Resultados finales
print("\n\n==== RESULTADOS GLOBALES ====")
print(f"Precisión total del algoritmo: {precision_global*100:.2f}%")
print(f"Aciertos totales: {aciertos_totales}/{total_recomendaciones}\n")

# Precisión por perfil
print("Precisión por perfil de usuario:")
for user_id, precision in resultados.items():
    print(f"- {user_id}: {precision*100:.1f}%")
